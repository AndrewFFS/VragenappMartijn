...
function getVragen() {
    ...
    $.getJSON(url, function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            // variable with sliced dateendtime in yyyy/mm/dd hh:mm format
            var slicedDateEndtime = data[i].eindTijd.slice(0, 16);

            // if the sliced datetime is current datetime or older than current datetime and if sliceddateendtime is current datetime or younger than current datetime execute if statement that checks if user meets requirements. If met, create open question
            if ((slicedDatetime == dateTime || slicedDatetime < dateTime) && (slicedDateEndtime == dateTime || slicedDateEndtime > dateTime)) {
